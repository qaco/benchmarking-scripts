...
#ifdef POLYBENCH_GEM5
#include "gem5/m5ops.h"
#endif

#ifdef POLYBENCH_PIPEDREAM
# include <perf-pipedream.h>
# define POLYBENCH_PAPI
# define PAPI_strerror perf_pipedream_strerror
# define PAPI_query_event perf_pipedream_query_event
# define PAPI_library_init perf_pipedream_library_init
# define PAPI_is_initialized perf_pipedream_is_initialized
# define PAPI_event_name_to_code perf_pipedream_event_name_to_code
# define PAPI_create_eventset perf_pipedream_create_eventset
# define PAPI_add_event perf_pipedream_add_event
# define PAPI_remove_event perf_pipedream_remove_event
# define PAPI_start perf_pipedream_start
# define PAPI_read perf_pipedream_read
# define PAPI_stop perf_pipedream_stop
# define PAPI_cleanup_eventset perf_pipedream_cleanup_eventset
# define PAPI_destroy_eventset perf_pipedream_destroy_eventset
# define PAPI_shutdown perf_pipedream_shutdown
# define PAPI_get_event_info perf_pipedream_get_event_info
# define PAPI_event_code_to_name perf_pipedream_event_code_to_name

static int perf_pipedream_event_code_to_name(int EventCode, char *out) {
  return PAPI_OK;
}

static int perf_pipedream_get_event_info(int code, void *ev) {
  return PAPI_OK;
}
#define PAPI_VER_CURRENT PERFPIPEDREAM_CURRENT_VERSION
...
void polybench_gem5_start()
{
  m5_work_begin(0,0);
}


void polybench_gem5_stop()
{
  m5_work_end(0,0);
}


void polybench_gem5_print()
{
...
# endif
	...
	for (k = 0; _polybench_papi_eventlist[k]; ++k)
	  {
	    printf("%s: ", _polybench_papi_eventlist[k]);
	    if ((retval =
